		...
		if(isset($_POST["editSale"])){
			...
			if($productChange){
				...
				foreach ($products as $key => $value) {				
					...
					$newSales = ModelProducts::mdlUpdateProduct($tableProducts, $item1a, $value1a, $value);

					$item1b = "stock";
					$value1b = $value["quantity"] + $getProduct["stock"];

					$stockNew = ModelProducts::mdlUpdateProduct($tableProducts, $item1b, $value1b, $value);
				...
				foreach ($productsList_2 as $key => $value) {
					...
					$newSales_2 = ModelProducts::mdlUpdateProduct($tableProducts_2, $item1a_2, $value1a_2, $value_2);

					$item1b_2 = "stock";
					$value1b_2 = $getProduct_2["stock"] - $value["quantity"];

					$newStock_2 = ModelProducts::mdlUpdateProduct($tableProducts_2, $item1b_2, $value1b_2, $value_2);
	...
	static public function ctrDeleteSale(){
		...
		if(isset($_GET["idSale"])){
			...
			foreach ($products as $key => $value) {
				...
				$stockNew = ModelProducts::mdlUpdateProduct($tableProducts, $item1b, $value1b, $valueProductId);
		...
		if(isset($_GET["report"])){
			...
			foreach ($sales as $row => $item){
			 ...
			 echo utf8_decode("<tr>
		 		...
		 		echo utf8_decode("</td>
					<td style='border:1px solid #eee;'>Tsh ".number_format($item["tax"],2)."</td>
					<td style='border:1px solid #eee;'>Tsh ".number_format($item["netPrice"],2)."</td>	
					<td style='border:1px solid #eee;'>Tsh ".number_format($item["totalPrice"],2)."</td>
